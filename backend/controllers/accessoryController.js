...
const updateAccessory = async (req, res) => {
  try {
    ...
    accessory.priceForKrishna = priceForKrishna
      ? Number(priceForKrishna)
      : accessory.priceForKrishna;
...
const deleteAccessory = async (req, res) => {
  try {
    ...
    if (accessory.imagePublicIds && accessory.imagePublicIds.length > 0) {
      await Promise.all(
        accessory.imagePublicIds.map((id) => cloudinary.uploader.destroy(id))
      );
    }
